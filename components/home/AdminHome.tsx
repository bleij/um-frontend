...
export default function AdminHome() {
  ...
  const selectedMentor: MentorApp | null =
    mentorApps.data.find((m) => m.id === selectedMentorId) ?? pendingMentors[0] ?? null;
  ...
  const statCards = [
    { label: "Ожидают проверки", value: String(stats.pendingMentors), icon: "file-text", color: "#CA8A04", bg: "#FEF9C3" },
    { label: "Всего менторов", value: String(stats.totalMentors), icon: "users", color: COLORS.primary, bg: COLORS.primary + "15" },
    { label: "Активных сессий", value: String(stats.activeSessions), icon: "clock", color: "#16A34A", bg: "#DCFCE7" },
    { label: "Доход", value: formatKZT(stats.revenue), icon: "dollar-sign", color: "#2563EB", bg: "#DBEAFE" },
  ];

  const NAV_ITEMS = [
    { id: "crm", label: "Пользователи & CRM", icon: "users", badge: stats.pendingMentors || undefined },
      ...
      <View style={{ flex: 1, flexDirection: isTablet ? "row" : "column" }}>
        <ScrollView style={{ flex: 1, backgroundColor: COLORS.background }}>
          ...
          {crmSubTab === "mentors" && !mentorApps.loading && filteredMentors.length === 0 && (
            <View style={{ padding: SPACING.lg }}><Text style={{ color: COLORS.mutedForeground }}>Заявок нет.</Text></View>
          )}
          ...
          {crmSubTab === "users" && !families.loading && families.data.length === 0 && (
            <View style={{ padding: SPACING.lg }}><Text style={{ color: COLORS.mutedForeground }}>Пока нет зарегистрированных родителей.</Text></View>
          )}
          {crmSubTab === "users" && !families.loading && families.data.map((f) => (
            <View
              key={f.id}
              style={{
                padding: SPACING.lg,
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={{ alignItems: "center", marginBottom: SPACING.xl }}>
                <View style={{ width: 80, height: 80, borderRadius: 40, backgroundColor: COLORS.primary + "15", alignItems: "center", justifyContent: "center", marginBottom: SPACING.sm }}>
                  <Text style={{ fontSize: 40 }}>{selectedMentor.photo_emoji}</Text>
                </View>
              ...
              {selectedMentor.status === "pending" && (
                <View style={{ gap: SPACING.sm }}>
                  <TouchableOpacity
                    ...
                    style={{ backgroundColor: COLORS.success, padding: SPACING.md, borderRadius: RADIUS.lg, alignItems: "center", ...SHADOWS.sm }}
                  >
                    <Text style={{ color: "white", fontWeight: TYPOGRAPHY.weight.bold }}>Одобрить</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={() => mentorApps.reject(selectedMentor.id)}
                    style={{ backgroundColor: COLORS.destructive, padding: SPACING.md, borderRadius: RADIUS.lg, alignItems: "center" }}
                  >
  ...
  const renderOrgsView = () => (
    <View style={{ flex: 1 }}>
      ...
      <ScrollView style={{ flex: 1, backgroundColor: COLORS.background }}>
        {orgs.loading && <View style={{ padding: SPACING.lg }}><Text style={{ color: COLORS.mutedForeground }}>Загрузка...</Text></View>}
        {!orgs.loading && orgs.data.length === 0 && <View style={{ padding: SPACING.lg }}><Text style={{ color: COLORS.mutedForeground }}>Организаций пока нет.</Text></View>}
        {orgs.data.map((org) => (
          <View
            key={org.id}
            style={{
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ fontSize: TYPOGRAPHY.size.xs, color: COLORS.mutedForeground, marginTop: 2 }}>
                Категория: {org.category ?? "—"} • Учеников: {org.active_students}
              </Text>
            </View>

            {org.status === "pending" ? (
              <View style={{ flexDirection: "row", gap: SPACING.sm }}>
                <TouchableOpacity onPress={() => orgs.verify(org.id)} style={{ backgroundColor: COLORS.success, paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm, borderRadius: RADIUS.md }}>
                  <Text style={{ color: "white", fontSize: TYPOGRAPHY.size.xs, fontWeight: "bold" }}>Верифицировать</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => orgs.reject(org.id)} style={{ backgroundColor: COLORS.destructive, paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm, borderRadius: RADIUS.md }}>
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={{ flexDirection: "row", gap: SPACING.md, paddingHorizontal: paddingX, marginVertical: SPACING.md }}>
        {(["tests", "tags", "logic"] as const).map((t) => (
          <TouchableOpacity key={t} onPress={() => setContentSubTab(t)} style={{ paddingVertical: 8, paddingHorizontal: 16, backgroundColor: contentSubTab === t ? COLORS.primary : COLORS.muted, borderRadius: RADIUS.full }}>
            <Text style={{ color: contentSubTab === t ? "white" : COLORS.foreground, fontWeight: "600" }}>
              {t === "tests" ? "Вопросы (Тест)" : t === "tags" ? "Справочник Тегов" : "AI Логика"}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <ScrollView style={{ flex: 1, backgroundColor: COLORS.background }}>
        {contentSubTab === "tests" && (
          <View style={{ paddingHorizontal: paddingX }}>
            {questions.loading && <Text style={{ color: COLORS.mutedForeground }}>Загрузка...</Text>}
            {!questions.loading && questions.data.length === 0 && <Text style={{ color: COLORS.mutedForeground }}>Вопросов нет.</Text>}
            {questions.data.map((q) => (
              <View key={q.id} style={{ padding: SPACING.lg, borderRadius: RADIUS.lg, backgroundColor: COLORS.surface, marginBottom: SPACING.md, borderWidth: 1, borderColor: COLORS.border, flexDirection: "row", alignItems: "center" }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: TYPOGRAPHY.size.md, fontWeight: "500", color: COLORS.foreground }}>{q.question}</Text>
                  {q.tag && <Text style={{ fontSize: TYPOGRAPHY.size.sm, color: COLORS.primary, marginTop: 4 }}>Привязка: {q.tag}</Text>}
                </View>
          ...
          <View style={{ padding: paddingX }}>
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: SPACING.md, marginBottom: SPACING.lg }}>
              {tags.data.map((tag) => (
                <TouchableOpacity
                  ...
                  style={{ paddingHorizontal: 16, paddingVertical: 8, borderRadius: RADIUS.full, backgroundColor: COLORS.primary + "15", borderWidth: 1, borderColor: COLORS.primary + "30" }}
                >
            ...
            <View style={{ flexDirection: "row", gap: SPACING.sm, alignItems: "center" }}>
              <TextInput
                ...
                style={{ flex: 1, padding: SPACING.md, borderRadius: RADIUS.md, borderWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.surface, color: COLORS.foreground }}
              />
              <TouchableOpacity
                onPress={async () => { await tags.add(newTagName); setNewTagName(""); }}
                style={{ paddingHorizontal: 16, paddingVertical: 10, borderRadius: RADIUS.md, backgroundColor: COLORS.primary }}
              >
                <Text style={{ color: "white", fontWeight: "bold" }}>+ Добавить</Text>
              </TouchableOpacity>
            </View>
            <Text style={{ fontSize: TYPOGRAPHY.size.xs, color: COLORS.mutedForeground, marginTop: SPACING.sm }}>Долгое нажатие на тег — удалить.</Text>
          </View>
        )}
        ...
        {contentSubTab === "logic" && (
          <View style={{ paddingHorizontal: paddingX }}>
            ...
            {aiRules.data.map((r) => (
              <View key={r.id} style={{ padding: SPACING.lg, backgroundColor: COLORS.surface, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: COLORS.border, flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: SPACING.md }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: "bold", fontSize: 16, color: COLORS.foreground }}>{r.name}</Text>
                  <Text style={{ color: COLORS.mutedForeground, marginTop: 4 }}>{r.condition}</Text>
                </View>
                <Feather name="arrow-right" size={24} color={COLORS.mutedForeground} style={{ marginHorizontal: 16 }} />
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: "bold", fontSize: 16, color: COLORS.primary }}>{r.recommendation_title}</Text>
                  {r.recommendation_body && <Text style={{ color: COLORS.mutedForeground, marginTop: 4 }}>{r.recommendation_body}</Text>}
                </View>
              </View>
            ))}
          </View>
    ...
    <View style={{ flex: 1 }}>
      <View style={{ padding: paddingX, borderBottomWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.surface }}>
        ...
        <View style={{ flexDirection: isTablet ? "row" : "column", gap: SPACING.md, paddingBottom: SPACING.lg }}>
          <View style={{ flex: 1, padding: SPACING.lg, borderRadius: RADIUS.lg, backgroundColor: COLORS.primary + "10", borderWidth: 1, borderColor: COLORS.primary + "30" }}>
            ...
            <Text style={{ fontSize: TYPOGRAPHY.size.xxl, fontWeight: "bold", color: COLORS.foreground, marginTop: 8 }}>{formatKZT(stats.gmv)}</Text>
          </View>
          <View style={{ flex: 1, padding: SPACING.lg, borderRadius: RADIUS.lg, backgroundColor: COLORS.success + "10", borderWidth: 1, borderColor: COLORS.success + "30" }}>
            <Text style={{ fontSize: TYPOGRAPHY.size.sm, color: COLORS.mutedForeground }}>Доход платформы</Text>
            <Text style={{ fontSize: TYPOGRAPHY.size.xxl, fontWeight: "bold", color: COLORS.success, marginTop: 8 }}>{formatKZT(stats.revenue)}</Text>
          </View>
          <View style={{ flex: 1, padding: SPACING.lg, borderRadius: RADIUS.lg, backgroundColor: COLORS.background, borderWidth: 1, borderColor: COLORS.border }}>
            ...
            <Text style={{ fontSize: TYPOGRAPHY.size.xxl, fontWeight: "bold", color: COLORS.foreground, marginTop: 8 }}>{stats.proSubscribers} / {stats.totalSubscribers}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={{ flexDirection: "row", gap: SPACING.md, paddingHorizontal: paddingX, marginVertical: SPACING.md }}>
        {(["transactions", "fees"] as const).map((t) => (
          <TouchableOpacity key={t} onPress={() => setBillingSubTab(t)} style={{ paddingVertical: 8, paddingHorizontal: 16, backgroundColor: billingSubTab === t ? COLORS.primary : COLORS.muted, borderRadius: RADIUS.full }}>
            <Text style={{ color: billingSubTab === t ? "white" : COLORS.foreground, fontWeight: "600" }}>{t === "transactions" ? "Сплит-Транзакции" : "Управление комиссией"}</Text>
          </TouchableOpacity>
        ))}
      </View>

      <ScrollView style={{ flex: 1, backgroundColor: COLORS.background }}>
        {billingSubTab === "transactions" && txs.loading && <View style={{ padding: SPACING.lg }}><Text style={{ color: COLORS.mutedForeground }}>Загрузка...</Text></View>}
        {billingSubTab === "transactions" && !txs.loading && txs.data.length === 0 && <View style={{ padding: SPACING.lg }}><Text style={{ color: COLORS.mutedForeground }}>Транзакций нет.</Text></View>}
        {billingSubTab === "transactions" && txs.data.map((t) => (
          <View key={t.id} style={{ padding: SPACING.lg, borderBottomWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.surface, flexDirection: "row", alignItems: "center" }}>
            ...
            <View style={{ flex: 1, marginLeft: SPACING.md }}>
              <Text style={{ fontSize: TYPOGRAPHY.size.md, fontWeight: "bold", color: COLORS.foreground }}>{t.parent_name} → {t.org_name}</Text>
              <Text style={{ fontSize: TYPOGRAPHY.size.xs, color: COLORS.mutedForeground, marginTop: 2 }}>{formatDate(t.created_at)} {t.external_ref ? `• ${t.external_ref}` : ""}</Text>
            </View>
            <View style={{ alignItems: "flex-end" }}>
              <Text style={{ fontSize: TYPOGRAPHY.size.md, fontWeight: "bold", color: COLORS.foreground }}>{formatKZT(t.amount)}</Text>
              <Text style={{ fontSize: TYPOGRAPHY.size.xs, color: COLORS.mutedForeground, marginTop: 2 }}>
                <Text style={{ color: COLORS.success }}>+{formatKZT(t.org_amount)} партнёру</Text> | <Text style={{ color: COLORS.primary }}>+{formatKZT(t.platform_amount)} платформе</Text>
              </Text>
            </View>
          </View>
        ))}

        {billingSubTab === "fees" && orgs.data.map((org) => (
            ...
            <View style={{ flexDirection: "row", alignItems: "center", backgroundColor: COLORS.background, borderWidth: 1, borderColor: COLORS.border, borderRadius: RADIUS.md, paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm }}>
              ...
              <TextInput
                ...
                onEndEditing={(e) => {
                  ...
                  if (Number.isFinite(val) && val !== org.commission_pct) orgs.setCommission(org.id, val);
                }}
                style={{ fontSize: TYPOGRAPHY.size.sm, fontWeight: "bold", color: COLORS.primary, width: 40, textAlign: "center" }}
              />
  ...
  const renderQCView = () => {
    const filtered = tickets.data.filter((t) => (qcSubTab === "tickets" ? t.kind === "complaint" : true));
    return (
      <View style={{ flex: 1 }}>
        <View style={{ padding: paddingX, borderBottomWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.surface }}>
          <View>
        ...
        <View style={{ flexDirection: "row", gap: SPACING.md, paddingHorizontal: paddingX, marginVertical: SPACING.md }}>
          {(["logs", "tickets"] as const).map((t) => (
            <TouchableOpacity key={t} onPress={() => setQcSubTab(t)} style={{ paddingVertical: 8, paddingHorizontal: 16, backgroundColor: qcSubTab === t ? COLORS.primary : COLORS.muted, borderRadius: RADIUS.full }}>
              <Text style={{ color: qcSubTab === t ? "white" : COLORS.foreground, fontWeight: "600" }}>{t === "logs" ? "Лог фидбеков" : "Тикеты и Жалобы"}</Text>
            </TouchableOpacity>
          ))}
        </View>

        <ScrollView style={{ flex: 1, backgroundColor: COLORS.background }}>
          {tickets.loading && <View style={{ padding: SPACING.lg }}><Text style={{ color: COLORS.mutedForeground }}>Загрузка...</Text></View>}
          {!tickets.loading && filtered.length === 0 && <View style={{ padding: SPACING.lg }}><Text style={{ color: COLORS.mutedForeground }}>Записей нет.</Text></View>}
          {filtered.map((tick) => {
            const kindLabel = tick.kind === "complaint" ? "Жалоба" : "Отзыв";
            const statusLabel = tick.status === "open" ? "Открыт" : tick.status === "in_progress" ? "В работе" : "Решен";
            const statusBg = tick.status === "open" ? "#FEF9C3" : tick.status === "in_progress" ? COLORS.primary + "20" : COLORS.success + "20";
            const statusColor = tick.status === "open" ? "#854D0E" : tick.status === "in_progress" ? COLORS.primary : COLORS.success;
            return (
              <View key={tick.id} style={{ padding: SPACING.lg, borderRadius: RADIUS.lg, marginHorizontal: paddingX, marginBottom: SPACING.md, backgroundColor: COLORS.surface, borderWidth: 1, borderColor: tick.kind === "complaint" ? COLORS.destructive + "30" : COLORS.border }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: SPACING.sm }}>
                  <View style={{ flexDirection: "row", gap: SPACING.sm }}>
                    <View style={{ backgroundColor: tick.kind === "complaint" ? COLORS.destructive + "15" : COLORS.primary + "15", paddingHorizontal: 8, paddingVertical: 2, borderRadius: RADIUS.sm }}>
                      <Text style={{ color: tick.kind === "complaint" ? COLORS.destructive : COLORS.primary, fontSize: 10, fontWeight: "bold", textTransform: "uppercase" }}>{kindLabel}</Text>
                    </View>
                    <Text style={{ fontSize: TYPOGRAPHY.size.xs, color: COLORS.mutedForeground }}>{formatDate(tick.created_at)}</Text>
                  </View>
                ...
                <Text style={{ fontSize: TYPOGRAPHY.size.md, fontWeight: "bold", color: COLORS.foreground, marginBottom: 4 }}>От: {tick.reporter_name}</Text>
                {tick.target && <Text style={{ fontSize: TYPOGRAPHY.size.xs, color: COLORS.mutedForeground, marginBottom: SPACING.md }}>Цель: {tick.target}</Text>}
                <Text style={{ fontSize: TYPOGRAPHY.size.sm, color: COLORS.foreground, lineHeight: 20 }}>{tick.body}</Text>

                <View style={{ flexDirection: "row", gap: SPACING.sm, marginTop: SPACING.md }}>
                  {tick.status === "open" && (
                    <TouchableOpacity onPress={() => tickets.takeInProgress(tick.id)} style={{ paddingVertical: SPACING.sm, paddingHorizontal: SPACING.md, backgroundColor: COLORS.primary, borderRadius: RADIUS.md }}>
                      <Text style={{ color: "white", fontWeight: "bold", fontSize: TYPOGRAPHY.size.sm }}>Взять в работу</Text>
                    </TouchableOpacity>
                  )}
                  {tick.status !== "resolved" && (
                    <TouchableOpacity onPress={() => tickets.resolve(tick.id)} style={{ paddingVertical: SPACING.sm, paddingHorizontal: SPACING.md, backgroundColor: COLORS.success, borderRadius: RADIUS.md }}>
                      <Text style={{ color: "white", fontWeight: "bold", fontSize: TYPOGRAPHY.size.sm }}>Отметить решённым</Text>
                    </TouchableOpacity>
                  )}
                </View>
              </View>
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "crm": return renderCRMView();
      case "orgs": return renderOrgsView();
      case "content": return renderContentView();
      case "billing": return renderBillingView();
      case "qc": return renderQCView();
      default: return null;
    }
  };
