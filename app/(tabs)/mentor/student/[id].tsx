    ...
    return (
        <View style={{ flex: 1, backgroundColor: "#F0FDF9" }}>
            ...
            <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
                ...
                {/* Student Card */}
                <View style={{ backgroundColor: "white", borderRadius: 22, padding: 20, alignItems: "center", marginBottom: 16, shadowColor: "#0D9488", shadowOpacity: 0.08, shadowRadius: 12 }}>
                    <View style={{ width: 80, height: 80, borderRadius: 40, backgroundColor: "#CCFBF1", alignItems: "center", justifyContent: "center", marginBottom: 12 }}>
                        <Text style={{ fontSize: 28, fontWeight: "800", color: "#0D9488" }}>{student.name.charAt(0)}</Text>
                    </View>
                    ...
                    <View style={{ flexDirection: "row", gap: 24 }}>
                        {[["XP", student.xp], ["Прогресс", `${student.progress}%`], ["Достижений", "12"]].map(([label, val]) => (
                            <View key={label} style={{ alignItems: "center" }}>
                                <Text style={{ fontSize: 22, fontWeight: "800", color: "#0D9488" }}>{val}</Text>
                    ...
                    <View style={{ flexDirection: "row", gap: 10, marginTop: 16, width: "100%" }}>
                        <TouchableOpacity
                            ...
                            style={{ flex: 1, backgroundColor: "#0D9488", borderRadius: 14, paddingVertical: 12, flexDirection: "row", alignItems: "center", justifyContent: "center" }}
                        >
                        ...
                        <TouchableOpacity
                            ...
                            style={{ backgroundColor: "#CCFBF1", borderRadius: 14, paddingHorizontal: 16, alignItems: "center", justifyContent: "center" }}
                        >
                    ...
                    {SKILLS.map(skill => (
                        <View key={skill.label} style={{ marginBottom: 10 }}>
                            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
                                ...
                                <Text style={{ fontSize: 13, fontWeight: "700", color: "#0D9488" }}>{skill.value}%</Text>
                            </View>
                            <View style={{ height: 6, backgroundColor: "#F3F4F6", borderRadius: 999 }}>
                                <View style={{ height: 6, width: `${skill.value}%`, backgroundColor: "#14B8A6", borderRadius: 999 }} />
                            </View>
                        </View>
                    ))}
                </View>
                    ...
                    <View style={{ flexDirection: "row", alignItems: "flex-end", height: 80, gap: 6 }}>
                        {XP_HISTORY.map((xp, i) => (
                            <View key={i} style={{ flex: 1, alignItems: "center" }}>
                                <View style={{ width: "100%", borderRadius: 6, height: (xp / maxXp) * 72, backgroundColor: "#14B8A6" }} />
                    ...
                    {TESTS.map((test, i) => (
                        <View key={i} style={{ padding: 12, backgroundColor: "#F9FAFB", borderRadius: 12, marginBottom: 8 }}>
                            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
                                ...
                                <Text style={{ fontSize: 20, fontWeight: "800", color: "#0D9488" }}>{test.score}%</Text>
                            </View>
                            <View style={{ height: 5, backgroundColor: "#E5E7EB", borderRadius: 999 }}>
                                <View style={{ height: 5, width: `${test.score}%`, backgroundColor: "#14B8A6", borderRadius: 999 }} />
                            </View>
                        </View>
                    ))}
                </View>
                ...
                <View style={{ backgroundColor: "white", borderRadius: 20, padding: 16, shadowColor: "#000", shadowOpacity: 0.04, shadowRadius: 8 }}>
                    ...
                    {RECS.map((rec, i) => (
                        <View key={i} style={{ padding: 12, borderRadius: 14, borderWidth: 2, borderColor: rec.priority === "high" ? "#0D9488" : "#E5E7EB", backgroundColor: rec.priority === "high" ? "#F0FDF9" : "#F9FAFB", marginBottom: 8 }}>
                            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
                                <Text style={{ fontWeight: "700", fontSize: 13, color: "#0D9488" }}>{rec.title}</Text>
                                {rec.priority === "high" && (
                                    <View style={{ backgroundColor: "#0D9488", paddingHorizontal: 8, paddingVertical: 2, borderRadius: 20 }}>
                    ...
                    <TouchableOpacity style={{ backgroundColor: "#CCFBF1", borderRadius: 12, padding: 12, alignItems: "center", marginTop: 4 }}>
                        <Text style={{ color: "#0D9488", fontWeight: "600", fontSize: 13 }}>+ Добавить рекомендацию</Text>
                    </TouchableOpacity>
                </View>
            </ScrollView>
        </View>
