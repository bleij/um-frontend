  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await register(
        ...
        password,  // Реальный пароль пользователя
        selectedRole,
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#6C5CE7] to-[#8B7FE8] flex flex-col">
      ...
      <div className="flex-1 bg-gray-50 rounded-t-[40px] px-6 py-8">
        <form onSubmit={codeSent ? (codeVerified ? handleSubmit : handleVerifyCode) : handleSendSms} className="space-y-4">
          ...
          {/* First Name (appears after verifying code) */}
          {codeVerified && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="relative">
          ...
          {/* Password (appears after verifying code) */}
          {codeVerified && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="relative">
                <Lock className="absolute left-4 top-4 w-5 h-5 text-[#6C5CE7] pointer-events-none" />
                <input
                ...
                <button
                  ...
                  className="absolute right-4 top-4 text-[#6C5CE7] hover:opacity-70 transition-opacity"
                >
          ...
          {/* Confirm Password (appears after verifying code) */}
          {codeVerified && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="relative">
                <Lock className="absolute left-4 top-4 w-5 h-5 text-[#6C5CE7] pointer-events-none" />
          ...
          <button
            type="submit"
            disabled={phoneNumber.replace(/\D/g, '').length < 11 || (codeSent && !smsCode) || (codeVerified && !firstName) || (codeVerified && !password) || (codeVerified && !confirmPassword)}
            className={`w-full py-4 rounded-2xl font-semibold text-lg shadow-lg transition-all mt-6 ${
              phoneNumber.replace(/\D/g, '').length < 11 || (codeSent && !smsCode) || (codeVerified && !firstName) || (codeVerified && !password) || (codeVerified && !confirmPassword)
