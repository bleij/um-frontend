...
import { createOrgCourse } from '../../lib/organization';
...
export function OrgCourseCreate() {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { orgProfile, loading: profileLoading } = useOrgProfile();
  
  console.log('🎯 OrgCourseCreate загружен');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('✅ OrgCourseCreate: создаем курс с профилем:', orgProfile);

      await createOrgCourse({
      ...
      navigate('/organization/courses');
    } catch (error) {
      console.error('❌ Ошибка создания курса:', error);
      alert('Ошибка при создании курса: ' + (error as Error).message);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-orange-50 to-yellow-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 shadow-lg max-w-md text-center">
          ...
          <p className="text-gray-600 mb-6">
            Не удалось загрузить профиль организации. Пожалуйста, вернитесь назад и попробуйте снова.
          </p>
          <button
            onClick={() => navigate(-1)}
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-yellow-50 pb-8">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-4 sticky top-0 z-10 shadow-lg">
        <div className="max-w-md mx-auto flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold">Создать курс</h1>
        </div>
      </div>
