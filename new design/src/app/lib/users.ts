...
export interface MentorProfile {
  ...
  photo_url?: string;
  city?: string;
  languages?: string; // JSON array ['Казахский', 'Русский', 'Английский']
  specialization?: string; // Психолог, Профориентолог, Тьютор по Hard Skills, Тренер по Soft Skills
  experience_years?: number;
  education?: string; // Учебное заведение
  certificate_url?: string; // Скан диплома
  skills?: string; // JSON array тегов ['#Лидерство', '#IT']
  pitch?: string; // Цепляющий заголовок (до 60 символов)
  bio?: string; // О с / О методе
  session_price?: number; // Стоимость сессии в тенге
  expertise?: string;
  phone?: string;
  created_at?: string;
  updated_at?: string;
...
export interface MentorPending {
  id: string;
  user_id: string;
  full_name: string;
  photo_url?: string;
...
export async function createChildProfile(data: Omit<ChildProfile, 'id' | 'created_at' | 'updated_at'>): Promise<ChildProfile> {
  ...
  if (!age) {
    ...
    age = !isNaN(ageFromCategory) ? ageFromCategory : 10; // Дефолтное значение, есл�� не число
  }

  await db.execute({
    sql: 'INSERT INTO child_profiles (id, parent_id, full_name, age, age_category, phone, user_id, interests, has_completed_test, test_result_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    args: [id, data.parent_id, data.full_name, age, data.age_category, data.phone || null, data.user_id || null, data.interests || null, data.has_completed_test || false, data.test_result_id || null],
  });
  return { id, ...data, age };
}
...
export async function createOrganizationProfile(data: Omit<OrganizationProfile, 'id' | 'created_at' | 'updated_at'>): Promise<OrganizationProfile> {
  ...
  console.log('📝 createOrganizationProfile: начало создания профиля');
  console.log('📋 Данные профиля:', data);
  console.log('🆔 Сгенерированный ID:', id);
  
  try {
    const result = await db.execute({
      sql: `INSERT INTO organization_profiles (
        id, user_id, organization_name, organization_type, bin, contact_person, 
        phone, email, city, address, description, license_url, registration_url, logo_url
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        id, 
        data.user_id, 
    ...
    console.log('✅ createOrganizationProfile: результат INSERT:', result);
    console.log('✅ createOrganizationProfile: профиль сохранен в БД');
    
    return { id, ...data };
  } catch (error) {
    console.error('❌ createOrganizationProfile: ошибка при INSERT:', error);
    throw error;
  }
}
...
export async function getOrganizationProfileByUserId(userId: string): Promise<OrganizationProfile | null> {
  ...
  console.log('🔍 getOrganizationProfileByUserId: поиск профиля для user_id:', userId);
  ...
  console.log('📦 getOrganizationProfileByUserId: результат запроса:', result.rows);
  console.log('📊 getOrganizationProfileByUserId: количество строк:', result.rows.length);
  
  if (result.rows.length === 0) {
    console.warn('⚠️ getOrganizationProfileByUserId: профиль не найден для user_id:', userId);
    return null;
  }
  
  console.log('✅ getOrganizationProfileByUserId: профиль найден:', result.rows[0]);
...
export async function createMentorProfile(data: Omit<MentorProfile, 'id' | 'created_at' | 'updated_at'>): Promise<MentorProfile> {
  const id = generateId();
  await db.execute({
    sql: `INSERT INTO mentor_profiles (
      id, user_id, full_name, photo_url, city, languages, specialization,
      experience_years, education, certificate_url, skills, pitch, bio,
      session_price, expertise, phone
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [
      id,
      data.user_id,
...
// ===== ОЖИДАЮЩИЕ МОДЕРАЦИИ МЕНТОРЫ =====

export async function createMentorPending(data: Omit<MentorPending, 'id' | 'status' | 'submitted_at'>): Promise<MentorPending> {
  const id = generateId();
  await db.execute({
    sql: `INSERT INTO mentors_pending (
      id, user_id, full_name, photo_url, city, languages, specialization,
      experience_years, education, certificate_url, skills, pitch, bio,
      session_price, phone
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
export async function getMentorPendingByUserId(userId: string): Promise<MentorPending | null> {
  const result = await db.execute({
    sql: 'SELECT * FROM mentors_pending WHERE user_id = ?',
    args: [userId],
  });
  if (result.rows.length === 0) return null;
  return result.rows[0] as unknown as MentorPending;
}